...
Delegate to the `commit-auditor` agent via the task tool (`subagent_type: "commit-auditor"`). It is
read-only — it reports, it does not repair, and it must leave the working tree unchanged.

Supply what you know:

- the plan (an absolute path to a repo document, or wherever plans are kept in this project). If you do
