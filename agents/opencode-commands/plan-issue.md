...
4. Delegate to the `plan-reviewer` agent via the task tool (`subagent_type: "plan-reviewer"`)
   adversarially against the plan. Fix every defect it finds.
5. ONLY THEN present the revised plan to the user and wait for approval before implementation begins.
