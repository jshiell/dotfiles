...
# Files generated by pattern rules (agents/claude-agents/%, agents/opencode-agents/%) must not be
# treated as disposable intermediates -- the symlinks in $(CLAUDE_AGENT_DIR)/$(OPENCODE_AGENT_DIR)
# point straight at them, so make must not delete them after building.
.SECONDARY:
...
$(DOTFILES)/agents/claude-agents/%.md: $(DOTFILES)/agents/definitions/%.md $(DOTFILES)/agents/scripts/generate-agent.py
	@mkdir -p $(DOTFILES)/agents/claude-agents
	python3 $(DOTFILES)/agents/scripts/generate-agent.py --target claude $< $@

$(DOTFILES)/agents/opencode-agents/%.md: $(DOTFILES)/agents/definitions/%.md $(DOTFILES)/agents/scripts/generate-agent.py
	@mkdir -p $(DOTFILES)/agents/opencode-agents
	python3 $(DOTFILES)/agents/scripts/generate-agent.py --target opencode $< $@
